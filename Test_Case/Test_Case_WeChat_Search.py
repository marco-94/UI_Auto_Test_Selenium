...
class Test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.brower.set_window_size(1920, 1080)
        self.brower.get("https://weixin.sogou.com/")
        time.sleep(3)
    ...
    def word_only_search(self):
        u"""单个关键词指定检索"""
        self.brower.find_element_by_xpath("//*[@id=\"query\"]").send_keys("微信")
        self.brower.find_element_by_xpath("//*[@id=\"searchForm\"]/div/input[3]").click()
        time.sleep(2)
    ...
    def word_random_search(self):
        ...
        for i in range(0, len(word_list)):
            ...
            print(word)
            search_key = self.brower.find_element_by_xpath("//*[@id=\"query\"]")
            time.sleep(1)
            search_key.clear()
            time.sleep(1)
            ...
            if i == 0:
                ...
                self.brower.find_element_by_xpath("//*[@id=\"scroll-header\"]/form/div/input[1]").click()
                time.sleep(3)
                self.save_img('取top10关键词，随机检索')
    ...
    def word_only_search(self):
        ...
        for i in range(0, 10):
            ...
            # 属下子节点的全部a标签
            self.brower.find_elements_by_xpath("//*[@id=\"topwords\"]/descendant::a")[j].click()
            self.save_img('点击top榜单话题随机检索')
            # 切换页面句柄
            handles = self.brower.window_handles
            self.brower.switch_to.window(handles[1])
            time.sleep(3)
            self.brower.close()
            handles = self.brower.window_handles
